...
cp "$BINARY" "$DL_DIR/mcwod_linux_amd64"
chmod +x "$DL_DIR/mcwod_linux_amd64"
( cd "$DL_DIR" && sha256sum mcwod_linux_amd64 > checksums.txt )
...
export MCWOD_API_BASE="$BASE"
export MCWOD_DOWNLOAD_BASE="$BASE"
: "${SUDO_USER:=root}"
export SUDO_USER
...
ok_if "unit grants CAP_NET_RAW, otherwise ICMP falls back to the ping binary" \
    grep -q "AmbientCapabilities=CAP_NET_RAW" /etc/systemd/system/mcwod.service
fail_if "the user placeholder was not substituted" \
    grep -q "MCWOD_USER" /etc/systemd/system/mcwod.service

echo ""
echo "=== 3. with a real config the service starts ==="
cat > /opt/mcwod/config.yml <<'CFGEOF'
watcher:
...
transfer:
  enabled: false
CFGEOF
chown "$SUDO_USER:$SUDO_USER" /opt/mcwod/config.yml 2>/dev/null || true
chmod 600 /opt/mcwod/config.yml

"$REPO_ROOT/watcher/install.sh" >"$WORK/install2.log" 2>&1 || true
for _ in $(seq 1 20); do
    if [ "$(systemctl is-active mcwod 2>&1)" = "active" ]; then
        break
    fi
    sleep 0.5
done

STATE="$(systemctl is-active mcwod 2>&1)"
ok_if "service is active (got $STATE)" test "$STATE" = "active"
ENABLED="$(systemctl is-enabled mcwod 2>&1)"
ok_if "service enabled at boot (got $ENABLED)" test "$ENABLED" = "enabled"
echo "  --- journal ---"
journalctl -u mcwod --no-pager -n 6 -o cat 2>/dev/null | indent
...
systemctl stop mcwod 2>/dev/null
printf 'tampered' >> "$DL_DIR/mcwod_linux_amd64"
...
( cd "$DL_DIR" && sha256sum mcwod_linux_amd64 > checksums.txt \
  && sed -i 's/mcwod_linux_amd64/something_else/' checksums.txt )
...
fail_if "unit removed" test -f /etc/systemd/system/mcwod.service
fail_if "install directory removed" test -d /opt/mcwod
